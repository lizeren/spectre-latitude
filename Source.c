...
char* secret = "The Magic Words are 123456 Ossifrage.";
...
void readMemoryByte(size_t malicious_x, uint8_t value[2], int score[2], int counter)
{
	...
	for (tries = 1; tries > 0; tries--)
	{
		...
		for (i = 0; i < 256; i++)
		{
			...
			time_diff = time2;
			printf("char %d has hit time:%ld\n",mix_i,time_diff);
			if(time_diff <= result_hit_time[counter] && mix_i != array1[tries % array1_size]){
	...
	value[1] = (uint8_t)k;
	score[1] = results[k];
	//printf("i:%c is %d, j:%c is %d\n",i,results[i],j,results[j]);
